...
- **Global scope**: `C:/Github/jelly-roll-proto/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Github/jelly-roll-proto/.gemini/notes/`)
...
2. Glob `C:/Github/jelly-roll-proto/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (C:/Github/jelly-roll-proto/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Github/jelly-roll-proto/.gemini/notes/` — works in any directory
